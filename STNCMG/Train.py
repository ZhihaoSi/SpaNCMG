...
def train_STNCMG(adata, hidden_dims=[512, 30], n_epochs=1000, lr=0.001,key_added='STNCMG',
    ...
    save_loss
        If True, the training loss is saved in adata.uns['STNCMG_loss'].
    save_reconstrction
        If True, the reconstructed expression profiles are saved in adata.layers['STNCMG_ReX'].
    device
        See torch.device.
    ...
    model = Mix_STNCMG(dim_input, dim_output).to(device)
    ...
    with torch.no_grad():
        ...
        if save_loss:
            adata.uns['STNCMG_loss'] = loss
        if save_reconstrction:
            ReX = emb.to('cpu').detach().numpy()
            ReX[ReX<0] = 0
            adata.layers['STNCMG_ReX'] = ReX   
        return adata 
